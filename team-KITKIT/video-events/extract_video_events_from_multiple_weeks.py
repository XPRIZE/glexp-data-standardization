...
#
# The extracted data will be stored in a file named `video-events-KITKIT.csv`.

import sys
import os

import extract_video_events_from_txt
...
for week_end_date in data_collection_week_end_dates:
    ...
    extract_video_events_from_txt.extract_from_week(directory_containing_weekly_data)

# Combine each CSV file for one week of data into one file
print(os.path.basename(__file__), "Writing data to \"video-events-KITKIT.csv\"...")
with open('video-events-KITKIT.csv', 'w') as outfile:
    infile_count = 0
    for week_end_date in data_collection_week_end_dates:
        print(os.path.basename(__file__), "\n\n"
                                          "**********\n")
        csv_filename_weekly = "video-events-KITKIT_" + week_end_date + ".csv"
        ...
        with open(csv_filename_weekly) as infile:
            ...
            for line in infile:
                ...
                infile_row_count += 1
        infile_count += 1
print(os.path.basename(__file__), "Writing data to \"video-events-KITKIT.csv\" complete!")
