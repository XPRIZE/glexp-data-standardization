# Collects video events from multiple weeks of data and combines them into one file.
#
# Example usage:
#     cd video-events
#     python3 extract_video_events_from_multiple_weeks.py ../tablet-usage-data
#
# The extracted data will be stored in a file named `video-events-ONEBILLION.csv`.

import sys
import os

import extract_video_events_from_db
...
data_collection_week_end_dates = [
    ...
    '2018-03-02', '2018-03-09', '2018-03-23', '2018-03-30',
...
# Extract video events and store them in a CSV file for each week of data
for week_end_date in data_collection_week_end_dates:
    ...
    extract_video_events_from_db.extract_from_week(directory_containing_weekly_data)

# Combine each CSV file for one week of data into one file
print(os.path.basename(__file__), "Writing data to \"video-events-ONEBILLION.csv\"...")
with open('video-events-ONEBILLION.csv', 'w') as outfile:
    infile_count = 0
    for week_end_date in data_collection_week_end_dates:
        print(os.path.basename(__file__), "\n\n"
                                          "**********\n")
        csv_filename_weekly = "video-events-ONEBILLION_" + week_end_date + ".csv"
        ...
        with open(csv_filename_weekly) as infile:
            ...
            for line in infile:
                ...
                infile_row_count += 1
        infile_count += 1
print(os.path.basename(__file__), "Writing data to \"video-events-ONEBILLION.csv\" complete!")
